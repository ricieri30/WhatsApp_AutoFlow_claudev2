...
async function resolveName(phone, fallback) {
  const clean = (x) => { const t = (x == null ? "" : String(x)).trim(); return (t && t !== ".") ? t : ""; };
  const ph = String(phone || "").replace(/\D/g, "");
  if (!ph) return clean(fallback);
  const alt = (ph.length === 13 && ph[4] === "9") ? ph.slice(0,4) + ph.slice(5) : null;
  const map = await __loadWaContacts();
  if (clean(map.get(ph))) return clean(map.get(ph));
  if (alt && clean(map.get(alt))) return clean(map.get(alt));
  if (clean(fallback)) return clean(fallback);
  try {
    const fc = await Contact.findOne({ phoneE164: { $in: alt ? [ph, alt] : [ph] } });
    if (fc && clean(fc.name)) return clean(fc.name);
  } catch (e) {}
  return "";
}
...
new Worker("wa-scheduler", async (job) => {
  ...
  if (job.name === "send-scheduled") {
    ...
    try {
      const schedName = await resolveName(msg.phoneE164, msg.contactName || msg.name || "");
        await sendText(msg.phoneE164, render(msg.message, { nome: (schedName && schedName.trim()) ? schedName.trim() : "voce" }));
  ...
  for (const c of targets) {
    ...
    await sendText(c.phoneE164, render(tpl.body, { nome: (c.name && c.name.trim()) ? c.name.trim() : "voce" }));
